    ...
    static sweetAlert2() {
        ...
        jQuery('.js-swal-confirm').on('click', e => {
            toast.fire({
                ...
                if (result.value) {
                    toast.fire('Удалено!', 'Успешно удалено.', 'success');
                    // result.dismiss can be 'overlay', 'cancel', 'close', 'esc', 'timer'
                } else if (result.dismiss === 'cancel') {
                    toast.fire('Отменено', 'Действие отменено', 'error');
                }
            })*/
            .then(result => {
                if (result.value) {
                    let elem_id = e.target.getAttribute('data-elemid');
                    let route_delete = e.target.getAttribute('data-delroute');
